...
mod tests {
    ...
    fn compact_target_from_upwards_difficulty_adjustment_using_headers() {
        ...
        // Blackcoin: recomputed from our Signet genesis bits. The 2016-block retargeting
        // algorithm is unchanged upstream code; only the genesis `bits`/`time` inputs differ
        // from Bitcoin's, so the expected output differs deterministically.
        let adjustment_bits = CompactTarget::from_consensus(503543726);
        assert_eq!(adjustment, adjustment_bits);
    }
